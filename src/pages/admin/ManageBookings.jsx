...
function ManageBookings() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const mappedData = data.map((b) => ({
        ...
        worker_name: b.worker?.name ?? "N/A",
        service: b.serviceCategory?.categoryName ?? "N/A",
        date: b.bookingDate
          ? new Date(b.bookingDate).toLocaleString()
          : "N/A",
        status: b.status ?? "pending",
        original: b, // keep original for edits
      }));
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // Prefer dedicated status endpoint
      ...
      try {
        await updateBookingApi(editRow.id, {
          ...editRow.original,
          status: statusValue,
        });
        await fetchBookings();
        ...
        setError(
          err2?.response?.data?.message || err2?.message || "Failed to update booking"
        );
      }
    } finally {
      setIsLoading(false);
